...
public class LichSuBanThuocController {
    ...
    private void cauHinhBangLichSuHoaDon() {
        ...
        colStt.setCellValueFactory(
                cellData -> new SimpleObjectProperty<>(tableLichSuBanHang.getItems().indexOf(cellData.getValue()) + 1));
    ...
    private void capNhatThongTinTongQuan(List<HoaDon> hoaDons) {
        ...
        BigDecimal trungBinh = tongHoaDon == 0
                ? BigDecimal.ZERO
                : tongDoanhThu.divide(BigDecimal.valueOf(tongHoaDon), 0, java.math.RoundingMode.HALF_UP);
        ...
        if (lblDoanhThu != null) {
            lblDoanhThu.setText(dinhDangTien(tongDoanhThu));
        }
        if (lblTrungBinhDon != null) {
            lblTrungBinhDon.setText(dinhDangTien(trungBinh));
        }
        if (lblHomNay != null) {
